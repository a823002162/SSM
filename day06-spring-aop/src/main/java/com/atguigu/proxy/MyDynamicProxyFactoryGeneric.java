 ...
 */
@Slf4j
public class MyDynamicProxyFactoryGeneric<T> {
    //被代理的对象
    private T target;

    //构造器
    public MyDynamicProxyFactoryGeneric(T target){
        this.target = target;
    }
    ...
    public T getProxy(){
        ...
        /*T proxy = (T)Proxy.newProxyInstance(classLoader, interfaces, new InvocationHandler() {
            @Override
